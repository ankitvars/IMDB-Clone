...
(function () {
	...
	const showFavourites = document.getElementById("favourites-section");
	...
	// Fetching the  data from api and calls function to add it in
	async function fetchMovies(search) {
		...
		try {
            ...
            // if there is any error in fetching url
			console.log(err);
		}
	}
	...
	function addToSuggestionContainerDOM(data) {
		...
		if (!isPresent && data.Title != undefined) {
			...
			movieCard.innerHTML = `
        <div class="card my-2 bg-dark text-white" data-id = " ${data.Title} ">
        <a href="movie.html" >
          <img
            src="${data.Poster} "
            class="card-img-top"
    ...
    <div class="movie-card-details">
      <p class="movie-name mt-3 mb-0">
       <a href = "movie.html" class="fs-4 text-success fav-movie-name" data-id="${movie.Title}">${movie.Title}<a> 
      </p>
      <small class="text-success fs-5">${movie.Year}</small>
    </div>
